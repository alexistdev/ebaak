...
<!DOCTYPE html>
<html>
<?php $this->load->view('mahasiswa/template/v_header') ?>

<body class="hold-transition sidebar-mini pace-danger">
<div class="wrapper">
	<!-- Site wrapper -->
	<?php $this->load->view('mahasiswa/template/v_navbar') ?>
	<?php $this->load->view('mahasiswa/template/v_sidebar') ?>
	<?php $this->load->view('mahasiswa/konten/k_kerjapraktek') ?>
	<?php $this->load->view('mahasiswa/template/v_footer') ?>
</div>
<!-- jQuery -->
...
<script>
	$(function() {
		$( "#tanggalPenelitian" ).datepicker({ dateFormat: 'dd-mm-yy' }).val();
		$( "#tanggalAkhir" ).datepicker({ dateFormat: 'dd-mm-yy' }).val();
	});
	$(function () {
		$("#tabelSidang").DataTable({
			"responsive": true,
			"autoWidth": false,
		});
	});
